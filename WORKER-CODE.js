...
export default {
  async fetch(request, env) {
      ...
      return json(
        {
          ...
          rateLimit:
            `${RATE_LIMIT_MAX} requests / 10 minutes`,
          realSsnEnabled: false,
          creditBureauConnected: false,
          message:
            "VeriCore secure sandbox API is working.",
    ...
    if (
      ...
      if (
        request.headers.get("X-VeriCore-Client") !==
        CLIENT_VERSION
      ) {
        return json(
          {
...
async function handleVerification(
  ...
  const consentReceipt =
    await createConsentReceipt(body);
  ...
  return json(
    {
      ...
      disclaimer:
        "Simulated result. Not returned by SSA, " +
        "Experian, Equifax, TransUnion, or any " +
...
function validateRequest(body, request) {
  ...
  const requestIdHeader = String(
    request.headers.get("X-Request-Id") || ""
  );

  const requestIdBody = String(
    body.requestId || ""
  );
  ...
  if (
    ...
    requestIdHeader !== requestIdBody
  ) {
    return "Invalid request identifier.";
  }

  if (
    !ALLOWED_CHECK_TYPES.has(
      String(body.checkType)
    )
  ) {
  ...
  if (
    ...
    return (
      "Sandbox accepts only Demo User / " +
      "1990-01-15 / 000-12-3456 / " +
      "demo@example.com. " +
      "Real personal data is blocked."
    );
  }

  const consentDate = new Date(
    String(body.consentTimestamp || "")
  );
  ...
  if (
    Math.abs(
      Date.now() - consentDate.getTime()
    ) >
    5 * 60 * 1000
  ) {
    return (
      "Consent timestamp is outside " +
...
async function enforceRateLimit(
  ...
  const salt = String(
    env.RATE_LIMIT_SALT ||
    "vericore-sandbox-v3.2"
  );

  const fingerprint =
    await sha256Hex(
      `${salt}|${ip}|${userAgent}|${windowId}`
    );
  ...
  const cacheKey = new Request(
    `${workerOrigin}/.__rate_limit/${fingerprint}`,
    {
      method: "GET"
    }
  );
...
function securityHeaders(allowedOrigin) {
  const headers = {
    ...
    "Access-Control-Allow-Headers":
      "Content-Type, Accept, " +
      "X-VeriCore-Client, X-Request-Id",
